...
The `dafishr` package was created to download, wrangle, and analyse Mexican VMS data. We strongly encourage to read `dafishr` documentation as well as this document. Many function are from the package and can be installed through CRAN with:

```{r eval=FALSE}
install.packages("dafishr")
```

In alternative, the development version can be installed using `devtools`:
...
To understand how much area was used we rasterized the modeled fishing area each month for the group of vessels that used Revillagigedo historically and the ones who did not. The script used to create the area used dataset is available in the `source` folder of this repository.
...
The `areas_results` dataset has three columns, featuring date (by month from 2009 to 2021) the area in km squared and whether data was from the vessels that historically fished in the Revillagigedo area ( `revi = "yes"` ) and does who did not (`revi = "no"`).
...
The raw script that can be applied to a full dataset processing is available in the `source` folder. Here, we provide the resulting rasters from before and after and explain the Normalized Fishing Index (NFI) calculation and reproduce the figure in main text.
...
We can then create a `NFI_gain` and `NFI_loss` objects representing the area gained and the area loss, respectively.
...
To visualize the resulting rasters we can transform them to points and then to a dataframe to be plotted with `ggplot2`.
...
The Causal Impact analysis results can be reproduced using the `causalimpact` package.
...
A full script that features the analysis for all figures is available in the `source` folder called `CausalImpact.R`.
